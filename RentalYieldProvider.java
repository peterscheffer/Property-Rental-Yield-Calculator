...
public class RentalYieldProvider
{
    ...
    public static void main( String [] args )
    {
		...
		for (String town : towns)
		{
			...
			List<Property> propertyValues = getPropertyValues("Buy", town, state);
	...
	private static List<Property> getPropertyValues (String type, String town, String state)
	{
		...
		String buyWebpageURL = "http://www.domain.com.au/Public/SearchResults.aspx?mode=buy&state=" + state +
								"&sub=" + URLEncoder.encode(town) + "&proptypes=1,2,4&ptdes=House%2c+Unit%2fFlat%2fApartment%2c+Townhouse&bedrooms=2,3,4,%3E5&extype=3&page=";
		String rentWebpageURL = "http://www.domain.com.au/Public/SearchResults.aspx?mode=rent&Refine=1&state=" + state +
								"&sub=" + URLEncoder.encode(town) + "&ssubs=0&bedrooms=%3E2&from=0&to=2147483647&proptypes=U,H,N&ptdes=House%2c+Unit%2fFlat%2fApartment%2c+Townhouse&page=";
		...
		numberOfPages = Math.round(Math.ceil((double)propertyListingCount / (double)PROPERTIES_PER_PAGE));
		...
		for (int index = 1; index <= numberOfPages; )
		{
			...
			while (index1 > 0) {
				...
				if (index6 > 0) {
					...
					bedrooms = details.substring(index7+1, details.indexOf(string4, index7));
					...
					{
						// Skip any where they can't enter the bedrooms correctly
						index1 = buffer.indexOf(string1, index2);
						continue;
					}
				}
				...
				if (index8 > 0) {
					...
					bathrooms = details.substring(index9+1, details.indexOf(string4, index9));
					...
					{
						// Skip any where they can't enter the bathrooms correctly
						index1 = buffer.indexOf(string1, index2);
						continue;
					}
				}
				...
				if (index10 > 0) {
					...
					propertyType = details.substring(index11+1, details.indexOf(string4, index11));
				...
				Property property = new Property(amount.toString(), bedrooms, bathrooms, propertyType);
	...
	private static String getWebpage (String webpageURL)
	{
		...
		try {
			...
			HttpURLConnection serverConnection = (HttpURLConnection)server.openConnection();
			...
			if (httpResponse == HttpURLConnection.HTTP_OK)
			{
				...
				BufferedInputStream in = new BufferedInputStream(serverConnection.getInputStream());
				int c;
	...
	private static double calculateAveragePrice(List<Property> properties, int bedroomCount, String propertyType)
	{
		double sum = 0f;
		int count = 0;
		...
		while (propertyList.hasNext())
		{
			...
			if (new Integer(property.getBedrooms()).equals(bedroomCount) && propertyType.equals(property.getPropertyType()))
			{
	...
	private static void createTable (String tableName)
	{
        Connection conn = null;
        Statement stmt = null;
        String createTable = "CREATE TABLE \"" + tableName + "\" (\"Buy or Rent\" TEXT, Type TEXT, Bedrooms TEXT, Bathrooms TEXT, Purchase TEXT, Rent TEXT, Mean TEXT, Variance TEXT)";
	...
	private static void writeToSpreadsheet (List<Property> properties, String buyOrRent, String town)
	{
		...
		String insertBuyOrRentHeader = "INSERT INTO [" + town.replaceAll(" ", "_") + "$] (\"Buy or Rent\") VALUES ('" + buyOrRent + "');";
		String insertBedroomCountHeader = "INSERT INTO [" + town.replaceAll(" ", "_") + "$] (Type) VALUES ('";
		String insertProperty = "INSERT INTO [" + town.replaceAll(" ", "_") + "$] (Bedrooms, Bathrooms, Purchase) VALUES ('";
		String insertMean = "INSERT INTO [" + town.replaceAll(" ", "_") + "$] (Mean, Variance) VALUES ('";
		String insertBlank = "INSERT INTO [" + town.replaceAll(" ", "_") + "$] (\"Buy or Rent\") VALUES (' ');";
		String insertSummary = "INSERT INTO [Yield$] (Town, Type, \"Mean Price\", \"Mean Rent\", Yield) VALUES ('";

		if (buyOrRent.equals("Rent")) {
			insertProperty = "INSERT INTO [" + town.replaceAll(" ", "_") + "$] (Bedrooms, Bathrooms, Rent) VALUES ('";
		}
        ...
        try
        {
			...
			for (int index=0; index<2; index++) {
				...
				for (; bedroomCount <= MAX_BEDROOMS; )
				{
					...
					Iterator propertyList = properties.iterator();
					double average = calculateAveragePrice(properties, bedroomCount, propertyType);

					// Insert each property into sheet based on # bedrooms and property type.
					while (propertyList.hasNext())
					{
						Property property = (Property)propertyList.next();
						if (new Integer(property.getBedrooms()).equals(bedroomCount) && property.getPropertyType().equals(propertyType))
						{
							hasProperties = true;
							String insertParams = property.getBedrooms() + "','" + property.getBathrooms() + "','" + nf.format(new Double(property.getPrice())) + "')";
							stmt.executeUpdate( insertProperty + insertParams );

							if (buyOrRent.equals("Buy")) {
								if (propertyType.equals("House"))
								{
									putMinMax(minHousePurchaseValue, maxHousePurchaseValue, bedroomCount, new Double(property.getPrice()));
								}
								else
								{
									putMinMax(minUnitPurchaseValue, maxUnitPurchaseValue, bedroomCount, new Double(property.getPrice()));
								}
							}
							else
							{
								if (propertyType.equals("House"))
								{
									putMinMax(minHouseRentValue, maxHouseRentValue, bedroomCount, new Double(property.getPrice()));
								}
								else
								{
									putMinMax(minUnitRentValue, maxUnitRentValue, bedroomCount, new Double(property.getPrice()));
								}
							}
						}
					}

					// If there are # bedroom properties, calculate averages and min/max.
					if (hasProperties)
					{
						if (buyOrRent.equals("Rent"))
						{
							if (propertyType.equals("House"))
							{
								meanHouseRentValues.put(new Integer(bedroomCount), new Double(average));

								double variance = 0.0;
								if (meanHouseRentValues.get(bedroomCount) != null)
								{
									variance = calculateVariance(minHouseRentValue, maxHouseRentValue, bedroomCount);
									stmt.executeUpdate( insertMean + nf.format(new Double(average)) + "', '" + yieldFormat.format(new Double(variance)) + "');");
								}

								double rentalYield = 0.0;
								if (meanHousePurchaseValues.get(bedroomCount) != null)
								{
									rentalYield = (average * 52) / meanHousePurchaseValues.get(bedroomCount);
									stmt.executeUpdate( insertSummary + town + "','" + bedroomCount + " Bedroom " + propertyType + "','" + nf.format(new Double(meanHousePurchaseValues.get(bedroomCount))) + "','" + nf.format(new Double(average)) + "','" + yieldFormat.format(rentalYield * 100) + "');");
								}
							}
							else if (propertyType.equals("Unit"))
							{
								meanUnitRentValues.put(new Integer(bedroomCount), new Double(average));

								double variance = 0.0;
								if (meanUnitRentValues.get(bedroomCount) != null)
								{
									variance = calculateVariance(minUnitRentValue, maxUnitRentValue, bedroomCount);
									stmt.executeUpdate( insertMean + nf.format(new Double(average)) + "', '" + yieldFormat.format(new Double(variance)) + "');");
								}
								...
								{
									rentalYield = (average * 52) / meanUnitPurchaseValues.get(bedroomCount);
									stmt.executeUpdate( insertSummary + town + "','" + bedroomCount + " Bedroom " + propertyType + "','" + nf.format(new Double(meanUnitPurchaseValues.get(bedroomCount))) + "','" + nf.format(new Double(average)) + "','" + yieldFormat.format(rentalYield * 100) + "');");
								}
							}
						}
						...
						if (buyOrRent.equals("Buy"))
						{
							if (propertyType.equals("House"))
							{
								double variance = calculateVariance(minHousePurchaseValue, maxHousePurchaseValue, bedroomCount);
								stmt.executeUpdate( insertMean + nf.format(new Double(average)) + "', '" + yieldFormat.format(new Double(variance)) + "');");

								meanHousePurchaseValues.put(new Integer(bedroomCount), new Double(average));
							}
							else
							{
								double variance = calculateVariance(minUnitPurchaseValue, maxUnitPurchaseValue, bedroomCount);
								stmt.executeUpdate( insertMean + nf.format(new Double(average)) + "', '" + yieldFormat.format(new Double(variance)) + "');");

								meanUnitPurchaseValues.put(new Integer(bedroomCount), new Double(average));
							}
						}
    ...
    static private double calculateVariance(Hashtable<Integer, Double> minHouseValue,
		Hashtable<Integer, Double> maxHouseValue, int bedroomCount)
	{
		double variance = 0.0;
		if (minHouseValue.get(bedroomCount) == null || maxHouseValue.get(bedroomCount) == null)
		{
			return variance;
		}
	...
	static private void putMinMax(Hashtable<Integer, Double> minHouseValue,
		Hashtable<Integer, Double> maxHouseValue, int bedroomCount, Double price)
	{
    ...
    private static class Property {
		...
		public Property (String price, String bedrooms, String bathrooms, String propertyType)
		{
			this.price = price;
